...
class ProjectManagerController {
    ...
    public function dashboard() {
        ...
        return [ 'success' => true, 'data' => compact('kpis','projects','tasks','openPOs','pendingMaterials') ];
    }
    ...
    public function listProjects() {
        ...
        // Fallback to owner if empty
        if (empty($projects) && method_exists($this->projectModel, 'getByOwner')) {
            $projects = $this->projectModel->getByOwner($userId) ?: [];
        }
        // Final fallback to all (createTask will still enforce permission)
        if (empty($projects)) {
            $projects = $this->safeCall($this->projectModel,'getAll',[]) ?: [];
        }
        // If still empty, derive projects from tasks (mirrors dashboard behavior)
        if (empty($projects)) {
            $tasks = [];
            if (method_exists($this->taskModel, 'getByManager')) {
                $tasks = $this->safeCall($this->taskModel, 'getByManager', [$userId, true]) ?: [];
            }
            if (empty($tasks)) {
                $tasks = $this->safeCall($this->taskModel, 'getAll', []) ?: [];
            }
            if (!empty($tasks)) {
                $byId = [];
                foreach ($tasks as $t) {
                    ...
                    if (!isset($byId[$pid])) {
                        $byId[$pid] = [
                            ...
                            'name' => $t['project_name'] ?? ('Project #'.$pid),
                            'status' => '',
                            'site_manager_name' => $t['site_manager_name'] ?? ''
                        ];
                    }
                }
                $projects = array_values($byId);
