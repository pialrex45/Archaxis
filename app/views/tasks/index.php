                        ...
                        <td colspan="6" class="text-center">
                            ...
                            <?php 
                              ...
                              try {
                                  ...
                                  $dbgRole = isset($_SESSION['user_role']) ? $_SESSION['user_role'] : (isset($_SESSION['role']) ? $_SESSION['role'] : 'none');
                                  $dbgUid = isset($_SESSION['user_id']) ? (int)$_SESSION['user_id'] : 0;
                                  echo '<div style="margin-top:8px;font-size:11px;opacity:0.7;">[debug role=' . htmlspecialchars($dbgRole) . ' uid=' . $dbgUid . ']</div>';
...
// Submit assign
// Resilient submit (supports both assignTaskSubmitBtn and assignTaskBtn ids)
function handleAssignSubmit(ev){
        ev.preventDefault();
        ...
        if (!form) { console.warn('[assign] form missing'); btn.disabled=false; delete btn.dataset.submitting; return; }
...
['assignTaskSubmitBtn','assignTaskBtn'].forEach(id=>{ const b=document.getElementById(id); if(b) b.addEventListener('click',handleAssignSubmit); });
