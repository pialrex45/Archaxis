...
function loadProjects() {
  fetch('<?= url('/api/pm/projects/list.php') ?>', {headers:{'X-Requested-With':'XMLHttpRequest'}})
    .then(r => r.json())
    .then(d => {
  ...
  let html = '<div class="table-responsive"><table class="table table-hover"><thead><tr><th>#</th><th>Name</th><th>Client</th><th>Status</th><th>Site Manager</th><th>Actions</th></tr></thead><tbody>';
      ...
      d.data.forEach(p => {
        ...
        const clientName = p.owner_name || p.client_name || '—';
        html += `
          <tr>
            <td>${projectId}</td>
            <td>${projectName}</td>
...
function loadSiteManagers() {
  ...
  return fetch('<?= url('/api/pm/site-managers/list.php') ?>', {headers:{'X-Requested-With':'XMLHttpRequest'}})
    .then(r => r.json())
    .then(d => {
...
document.addEventListener('DOMContentLoaded', function() {
  loadProjects();
  // Reparent modals to <body> to avoid stacking context / backdrop-filter issues
  ['editProjectModal','assignSiteManagerModal'].forEach(id => {
