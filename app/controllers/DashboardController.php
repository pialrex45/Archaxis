...
class DashboardController {
    ...
    public function getClientDashboard() {
        try {
            ...
            // Projects visible to client: prefer dedicated method, then ownership; do NOT slice here
            $projects = [];
            ...
            $tasks = [];
            if (!empty($projects) && method_exists($this->taskModel, 'getByProject')) {
                foreach ($projects as $proj) {
                    $projId = $proj['id'] ?? null;
                    if ($projId) {
                        $projTasks = $this->taskModel->getByProject($projId) ?: [];
                        foreach ($projTasks as $t) { $tasks[] = $t; }
                    }
                }
            }
            // Never pull global tasks for client without ownership
            ...
            $recentProjectsClient = array_map(function($p) {
                return [
                    ...
                    'name' => $p['name'] ?? ($p['title'] ?? ''),
                    'status' => $p['status'] ?? 'n/a',
                    'deadline' => $p['deadline'] ?? $p['end_date'] ?? ($p['due_date'] ?? null),
                ...
                'data' => [
                    ...
                    'projects' => array_map(function($p) {
                        return [
                            ...
                            'name' => $p['name'] ?? ($p['title'] ?? ''),
                            'status' => $p['status'] ?? 'n/a',
                            'deadline' => $p['deadline'] ?? $p['end_date'] ?? ($p['due_date'] ?? ''),
                        ];
                    }, $projects),
    ...
    public function getManagerDashboard() {
        try {
            ...
            // Get projects where this manager is owner OR assigned (site_manager / project_assignments)
            $projects = [];
    ...
    public function getSupervisorDashboard() {
        try {
            ...
            // Primary set: tasks explicitly assigned to this supervisor (legacy single-owner model)
            $assignedTasks = $this->taskModel->getAssignedTo($userId) ?: [];

            // Fallback: if none found (common with new multi-worker assignment model where supervisor is coordinator, not assignee)
            if (count($assignedTasks) === 0) {
                if (method_exists($this->taskModel, 'getRecentWithProject')) {
                    $assignedTasks = $this->taskModel->getRecentWithProject(12) ?: [];
                }
            }

            $totalTasks = count($assignedTasks);

            // Count task statuses over the chosen set
            $completedTasks = 0; $inProgressTasks = 0; $pendingTasks = 0;
            foreach ($assignedTasks as $task) {
                $st = isset($task['status']) ? strtolower($task['status']) : '';
                if ($st === 'completed') $completedTasks++; elseif ($st === 'in progress') $inProgressTasks++; elseif ($st === 'pending') $pendingTasks++;
            }
            ...
            return [
                ...
                'data' => [
                    'tasks' => [
                        ...
                        'pending' => $pendingTasks,
                        // show up to 12 (or fewer) recent tasks so supervisor has data even without direct assignment
                        'recent' => array_slice($assignedTasks, 0, 12)
                    ],
                    'recent_messages' => $recentMessages,
    ...
    // Minimal site manager dashboard (stub to prevent missing method error)
    public function getSiteManagerDashboard() {
        try {
            $userId = getCurrentUserId();
            if(!$userId) return ['success'=>false,'message'=>'User not authenticated'];
            // Reuse manager logic but restrict to projects where user is site manager when possible
            $projects = [];
            if (method_exists($this->projectModel,'getBySiteManager')) {
                $projects = $this->projectModel->getBySiteManager($userId) ?: [];
            } elseif (method_exists($this->projectModel,'getByOwner')) {
                $projects = $this->projectModel->getByOwner($userId) ?: [];
            }
            $tasks = [];
            if (!empty($projects) && method_exists($this->taskModel,'getByProject')) {
                foreach($projects as $p){ $pid=$p['id']??null; if($pid){ $tasks=array_merge($tasks,$this->taskModel->getByProject($pid)?:[]); } }
            }
            return [ 'success'=>true, 'data'=>[
                'projects'=>['total'=>count($projects),'list'=>array_slice($projects,0,5)],
                'tasks'=>['total'=>count($tasks),'recent'=>array_slice($tasks,0,10)],
            ]];
        } catch (Throwable $e){ return ['success'=>false,'message'=>'Error fetching site manager dashboard']; }
    }
