...
class TaskController {
    ...
    public function getAll() {
        try {
            ...
            $rawRole = getCurrentUserRole();
            $userRole = normalizeRole($rawRole);
            $userId = getCurrentUserId();
            
            // Get tasks based on normalized user role
            if (in_array($userRole, ['admin', 'manager'])) {
                $tasks = $this->taskModel->getAll();
            } elseif ($userRole === 'site_manager') {
                $tasks = $this->taskModel->getByManager($userId) ?: [];
                // Fallback: if none returned, also include directly assigned tasks
                if (empty($tasks)) {
                    $fallbackAssigned = $this->taskModel->getAssignedTo($userId) ?: [];
                    if (!empty($fallbackAssigned)) {
                        $tasks = $fallbackAssigned;
                    }
                }
            } elseif ($userRole === 'project_manager') {
                // Leverage Task model's project_manager logic via getByManager
                $tasks = $this->taskModel->getByManager($userId) ?: [];
            } else {
                $tasks = array_merge(
    ...
    public function updateStatus($taskId, $status) {
        try {
            ...
            if (!in_array($userRole, ['admin', 'manager'])) {
                ...
                if ((int)$task['assigned_to'] === (int)$userId) { $allowed = true; }
                // Fetch project for broader role checks
                if (!$allowed) {
                    $project = $this->projectModel->getById($task['project_id']);
                    if ($project) {
                        // Project owner
                        if ((int)$project['owner_id'] === (int)$userId) { $allowed = true; }
                        // Site manager (if column exists and matches current user)
                        if (!$allowed && normalizeRole($userRole) === 'site_manager' && isset($project['site_manager_id']) && (int)$project['site_manager_id'] === (int)$userId) {
                            $allowed = true; }
                    }
                }
                ...
                if (!$allowed) {
                    try {
                        $db = Database::getConnection();
                        $stmt = $db->prepare('SELECT 1 FROM project_assignments WHERE project_id = :pid AND user_id = :uid LIMIT 1');
                        $stmt->execute([':pid' => (int)$task['project_id'], ':uid' => (int)$userId]);
                        if ($stmt->fetchColumn()) { $allowed = true; }
                    } catch (Throwable $e) { /* ignore */ }
                }
                if (!$allowed) {
                    return ['success' => false, 'message' => 'Insufficient permissions to update this task', 'debug_role' => $userRole, 'debug_user' => $userId, 'debug_task' => $taskId];
                }
            }
