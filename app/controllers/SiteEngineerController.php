...
class SiteEngineerController {
    ...
    public function materials($projectId = null, $limit = 100) {
        ...
        $limit = max(1, min(200, (int)$limit));
        if ($projectId) {
            // Allow direct project view when explicitly requested and user has access; include project name for UI
            if (!empty($pids) && !in_array((int)$projectId, $pids, true)) { return ['success'=>false,'message'=>'Forbidden']; }
            $stmt = $this->pdo->prepare("SELECT m.*, p.name AS project_name FROM materials m JOIN projects p ON p.id = m.project_id WHERE m.project_id = ? ORDER BY m.created_at DESC LIMIT {$limit}");
            $stmt->execute([(int)$projectId]);
            return ['success'=>true,'data'=>$stmt->fetchAll() ?: []];
        }
        if (empty($pids)) {
            // Fallback: show recent materials across all projects (useful when no tasks are assigned yet)
            $stmt = $this->pdo->query("SELECT m.*, p.name AS project_name FROM materials m JOIN projects p ON p.id = m.project_id ORDER BY m.created_at DESC LIMIT {$limit}");
            return ['success'=>true,'data'=>$stmt->fetchAll() ?: []];
        }
        $in = implode(',', array_fill(0, count($pids), '?'));
        $stmt = $this->pdo->prepare("SELECT m.*, p.name AS project_name FROM materials m JOIN projects p ON p.id = m.project_id WHERE m.project_id IN ($in) ORDER BY m.created_at DESC LIMIT {$limit}");
    ...
    public function purchaseOrders($projectId = null, $limit = 100) {
        ...
        if ($projectId) {
            if (!empty($pids) && !in_array((int)$projectId, $pids, true)) { return ['success'=>false,'message'=>'Forbidden']; }
            $stmt = $this->pdo->prepare("SELECT po.id, po.project_id, p.name AS project_name, po.supplier_id, s.name AS supplier_name, po.status, po.total_amount, po.created_at
        ...
        if (empty($pids)) {
            $stmt = $this->pdo->query("SELECT po.id, po.project_id, p.name AS project_name, po.supplier_id, s.name AS supplier_name, po.status, po.total_amount, po.created_at
                                         FROM purchase_orders po
                                         JOIN projects p ON p.id = po.project_id
                                    LEFT JOIN suppliers s ON s.id = po.supplier_id
                                     ORDER BY po.id DESC LIMIT {$limit}");
