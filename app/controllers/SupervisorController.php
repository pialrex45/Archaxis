...
require_once __DIR__ . '/../models/TaskActivity.php';
...
class SupervisorController {
    ...
    private $taskActivityModel; // logbook
    ...
    $this->taskActivityModel = class_exists('TaskActivity') ? new TaskActivity() : null;
    }
    ...
    public function taskUpdateStatus($taskId, $status, $note = null, $photoPath = null) {
        ...
        if ($this->taskActivityModel) {
            try {
                $row = $this->taskModel->getById((int)$taskId);
                $pid = $row ? (int)$row['project_id'] : null;
                $summary = 'Status changed to ' . $status;
                $details = $note || $photoPath ? json_encode(['note'=>$note,'photo'=>$photoPath]) : null;
                $this->taskActivityModel->add((int)$taskId,$pid,$uid,null,'status',$summary,$details,$status,null);
    ...
    public function taskAddProgress($taskId, $note = null, $status = null, $photoPath = null) {
        ...
        if ($this->taskActivityModel) {
            try {
                ...
                $details = json_encode(['note'=>$note,'status'=>$status,'photo'=>$photoPath]);
                $this->taskActivityModel->add((int)$taskId,$pid,$uid,null,'progress',$summary,$details,$status,null);
            } catch (Throwable $e) { /* ignore */ }
        }
        return ['success'=>true,'message'=>'Progress added'];
    }
