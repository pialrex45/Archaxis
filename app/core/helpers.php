...
if (!function_exists('url')) {
    function url($path = '') {
        static $base = null;
        if ($base === null) {
            // Prefer APP_URL path component
            $raw = defined('APP_URL') ? APP_URL : (function_exists('env') ? env('APP_URL','') : '');
            $pathPart = $raw ? (parse_url(trim($raw), PHP_URL_PATH) ?: '') : '';
            if ($pathPart && $pathPart !== '/') { $base = rtrim($pathPart,'/'); }
            // Fallback to project folder name
            if (!$base && defined('BASE_PATH')) {
                $folder = basename(BASE_PATH);
                if ($folder && $folder !== 'public') { $base = '/' . $folder; }
            }
            if (!$base) { $base = ''; }
        }
