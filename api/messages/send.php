...
try {
    ...
    // Validate input
    if (empty($receiverId)) {
        jsonResponse(['success' => false, 'message' => 'Receiver ID is required'], 400);
    }
    ...
    if ($senderId == $receiverId) {
        jsonResponse(['success' => false, 'message' => 'Cannot send message to yourself'], 400);
    }
    ...
    if (!$policy->canDM($senderId, $receiverId)) {
        jsonResponse(['success' => false, 'message' => 'You are not allowed to message this user'], 403);
    }
    ...
    jsonResponse(['success' => false, 'message' => 'Error sending message: ' . $e->getMessage()], 500);
}
