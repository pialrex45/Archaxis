...
// Execute project assignments table migration (additive chain support)
echo "Executing project assignments migration...\n";
$assignmentsSqlFile = __DIR__ . '/migrations/add_project_assignments_table.sql';
if (file_exists($assignmentsSqlFile)) {
    try {
        $sql = file_get_contents($assignmentsSqlFile);
        $pdo->exec($sql);
        echo "Project assignments migration completed successfully.\n";
    } catch (PDOException $e) {
        echo "Error executing project assignments migration: " . $e->getMessage() . "\n";
    }
} else {
    echo "Project assignments migration file not found (skipped).\n";
}

// Execute project messages migration (per-project chats)
echo "Executing project messages migration...\n";
$projMsgSqlFile = __DIR__ . '/migrations/2025_09_add_project_messages.sql';
if (file_exists($projMsgSqlFile)) {
    try {
        $sql = file_get_contents($projMsgSqlFile);
        $pdo->exec($sql);
        echo "Project messages migration completed successfully.\n";
    } catch (PDOException $e) {
        echo "Error executing project messages migration: " . $e->getMessage() . "\n";
    }
} else {
    echo "Project messages migration file not found (skipped).\n";
}
...
// Execute designs workflow table migration
echo "Executing designs workflow migration...\n";
$designsSqlFile = __DIR__ . '/migrations/20251008_create_designs_table.sql';
if (file_exists($designsSqlFile)) {
    try {
        $sql = file_get_contents($designsSqlFile);
        $pdo->exec($sql);
        echo "Designs workflow migration completed successfully.\n";
    } catch (PDOException $e) {
        echo "Error executing designs workflow migration: " . $e->getMessage() . "\n";
    }
} else {
    echo "Designs workflow migration file not found (skipped).\n";
}

// Execute designs logs table migration
echo "Executing designs logs migration...\n";
$dlogsSqlFile = __DIR__ . '/migrations/20251008_create_designs_logs_table.sql';
if (file_exists($dlogsSqlFile)) {
    try {
        $sql = file_get_contents($dlogsSqlFile);
        $pdo->exec($sql);
        echo "Designs logs migration completed successfully.\n";
    } catch (PDOException $e) {
        echo "Error executing designs logs migration: " . $e->getMessage() . "\n";
    }
} else {
    echo "Designs logs migration file not found (skipped).\n";
}
